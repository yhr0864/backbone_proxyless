...
from building_blocks.layers import misc
...
def weights_init(m, deepth=0, max_depth=2):
    if deepth > max_depth:
        return
    if isinstance(m, misc.Conv2d):
        torch.nn.init.kaiming_uniform_(m.weight.data)
        if m.bias is not None:
            torch.nn.init.constant_(m.bias.data, 0)
    elif isinstance(m, misc.BatchNorm2d):
...
# Create an ARCH=model specification (see building_blocks/modeldef.py)
# and add the ARCH into MODEL_ARCH of building_blocks/modeldef.py with name "my_unique_name_for_ARCH"
...
#       and look into building_blocks/modeldef.py
...
def writh_new_ARCH_to_modeldef(arch_operations, my_unique_name_for_ARCH):

    ops_names = arch_operations['arch_operations_backbone']
    ops_names_head26 = arch_operations['arch_operations_head26']
    ops_names_head13 = arch_operations['arch_operations_head13']
    ops_names_fpn = arch_operations['arch_operations_fpn']

    assert len(ops_names) == 11
    assert len(ops_names_head26) == 5
    assert len(ops_names_head13) == 5
    assert len(ops_names_fpn) == 8

    if my_unique_name_for_ARCH in MODEL_ARCH:
        print("The specification with the name", my_unique_name_for_ARCH, "already written \
              to the building_blocks.modeldef. Please, create a new name \
              or delete the specification from building_blocks.modeldef (by hand)")
    ...
    ops_lines = [ops[0], ops[1:4], ops[4:7], ops[7:10], ops[10]]
    ops_lines = [''.join(line) for line in ops_lines]
    text_to_write += '            ' + '\n            '.join(ops_lines)
    text_to_write += "\n\
            ],\n"

    text_to_write += "\"block_op_type_head26\": [\n"
    ops = ["[\"" + str(op) + "\"], " for op in ops_names_head26]
    ops_lines = [ops[0:2], ops[2:4], ops[4]]
    ops_lines = [''.join(line) for line in ops_lines]
    text_to_write += '            ' + '\n            '.join(ops_lines)
    text_to_write += "\n\
                ],\n"

    text_to_write += "\"block_op_type_head13\": [\n"
    ops = ["[\"" + str(op) + "\"], " for op in ops_names_head13]
    ops_lines = [ops[0:2], ops[2:4], ops[4]]
    ops_lines = [''.join(line) for line in ops_lines]
    text_to_write += '            ' + '\n            '.join(ops_lines)
    text_to_write += "\n\
    ...
    text_to_write += "\"block_op_type_fpn\": [\n"
    ops = ["[\"" + str(op) + "\"], " for op in ops_names_fpn]
...
"
    ...
    with open('./building_blocks/modeldef.py') as f1:
        lines = f1.readlines()
    ...
    with open('./building_blocks/modeldef.py', 'w') as f2:
