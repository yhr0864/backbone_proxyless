...
class TrainerSupernet:
    def __init__(self, criterion, w_optimizer, theta_optimizer, w_scheduler, 
                 scale_sparse_rate, train_thetas_from_the_epoch, logger, writer):
        self.logger = logger
        self.writer = writer
    ...
    def weight_step(self, model, loader, optimizer, epoch, info_for_logger=''):
        ...
        for step, (_, images, targets) in enumerate(loader):
            ...
            # print(model.module_list[1].active_index)
            # print(model.module_list[1].ops[model.module_list[1].inactive_index[0]])
            ...
            #print("outs", outs[0])
            #print("latency", latency_to_accumulate)
    ...
    def gradient_step(self, model, loader, optimizer, epoch, step, info_for_logger=''):
        ...
        model.train()
        # Mix edge mode
        MixedOperation.MODE = CONFIG_SUPERNET['binary_mode']  # 默认设为two_v2
